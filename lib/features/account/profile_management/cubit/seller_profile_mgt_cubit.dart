...
class SellerProfileMgtCubit extends Cubit<SellerProfileMgtState> {
  SellerProfileMgtCubit(this._profileRepository)
      : super(SellerProfileMgtState.initial());
  ...
  Future<void> getProfileDetails([bool silent = false]) async {
    ...
    await result.when(
      ...
      failure: (error) {
        if (!silent) {
          state.copyWith(profileDetails: ProcessState.error(error));
        }
      },
    );
  }

  Future<void> getStylistStats() async {
    ...
    await result.when(
      ...
      failure: (error) {
        state.copyWith(stylistStats: ProcessState.error(error));
      },
    );
  }
